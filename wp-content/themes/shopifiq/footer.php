<?php
    ...
    <?php
    $google_maps_api = get_option('anps_google_map_api', '');

    if( $google_maps_api != '' ) {
        $google_maps_api = '?key=' . $google_maps_api;
    }
    wp_enqueue_script( "google_maps_api", "https://maps.google.com/maps/api/js" . $google_maps_api, '', '', true);
    ?>
    ...
    <?php if(!isset($_COOKIE["shopifiq-notification-closed"]) || get_option("notification_changes") != $_COOKIE["shopifiq-notification-closed"]): ?>
